...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="LLaVA-OneVision eval with built-in Zoom tool (no RAG).")
    p.add_argument("--model-path", type=str, required=True)
    p.add_argument("--name", type=str, default="agent_zoom_llava_onevision")
    ...
    p.add_argument("--limit_mm_images", type=int, default=12, help="Max number of images allowed per prompt.")
    ...
    p.add_argument("--enable_working_memory", action="store_true", help="Enable internal working memory module.")
    p.add_argument("--memory_max_chars", type=int, default=240, help="Max chars stored for each memory step.")
...
def _image_tokens(n: int) -> str:
    n = max(0, int(n))
    return "\n".join(["<image>"] * n) if n > 0 else ""


def render_chat_prompt(tokenizer: Any, user_text: str) -> str:
    """
    Render model-specific chat prompt.
    For LLaVA-style templates, `content` should be plain string (not list).
    """
    msgs = [{"role": "user", "content": user_text}]
    try:
        return tokenizer.apply_chat_template(msgs, tokenize=False, add_generation_prompt=True)
    except Exception:
        # Fallback to raw prompt in case tokenizer has no chat template.
        return user_text


def build_stage1_prompt_text(has_fewshot: bool, few_num: int, question_text: str) -> str:
    parts: List[str] = []
    if has_fewshot and few_num > 0:
        parts.append(
            f"Following are {few_num} normal reference image(s) for comparison:\n{_image_tokens(few_num)}"
        )
    parts.append("Now the test image:\n<image>")
    parts.append(
        f"{question_text}\n"
...
def build_stage2_prompt_text(
    ...
    parts.append("Test full image:\n<image>")
    parts.append("Zoomed crop of suspicious region:\n<image>")
    parts.append(
        f"{question_text}\n"
        f"Working memory:\n{memory_summary}\n"
...
def build_self_check_prompt_text(
    question_text: str,
    stage1_text: str,
    stage2_text: str,
    used_zoom: bool,
    ...
    parts: List[str] = ["Test full image:\n<image>"]
    if used_zoom:
        parts.append("Zoomed suspicious crop:\n<image>")
...
def run_batch_samples(
    ...
    for i, item in enumerate(batch_items):
        ...
        prompt1_text = build_stage1_prompt_text(
            has_fewshot=len(few_imgs) > 0,
            few_num=len(few_imgs),
            question_text=q_text,
        )
        prompt1 = render_chat_prompt(tokenizer, prompt1_text)
    ...
    for c, out in zip(ctx, outputs1):
        ...
        if enable_working_memory:
            c["working_memory"]["step1_global"] = _clip_text(text1, memory_max_chars)
        ...
        prompt2_text = build_stage2_prompt_text(
        ...
        prompt2 = render_chat_prompt(tokenizer, prompt2_text)
        ...
        try:
            ...
            for c in ctx:
                if c["idx"] in out2_map:
                    ...
                    if enable_working_memory and c["used_zoom"]:
                        c["working_memory"]["step2_zoomcrop"] = _clip_text(out2_map[c["idx"]], memory_max_chars)
        except Exception as e:
            for c in ctx:
                if c["used_zoom"]:
    ...
    if enable_self_check:
        ...
        for c in ctx:
            ...
            prompt_sc_text = build_self_check_prompt_text(
                question_text=c["q_text"],
            ...
            prompt_sc = render_chat_prompt(tokenizer, prompt_sc_text)
            ...
            try:
                ...
                for c in ctx:
                    if c["idx"] in sc_map:
                        ...
                        if enable_working_memory:
                            c["working_memory"]["step3_selfcheck"] = _clip_text(sc_map[c["idx"]], memory_max_chars)
