...
MASTER_PORT=$(/mnt/nfs/wmd/conda_envs/sft/bin/python - <<'PY'
import socket
s = socket.socket()
s.bind(("", 0))
...
/mnt/nfs/wmd/conda_envs/sft/bin/torchrun \
  ...
  --stage sft \
  --do_train \
  --model_name_or_path "/mnt/nfs/wmd/model/IAD-R1-UPDATE/IAD-R1(LLaVA-OneVision-SI-7B)" \
  --dataset_dir /mnt/nfs/wmd/research/MemRefine-IAD/agent/real_build \
  --dataset MemRefine_sft \
  --image_dir / \
  --template llava_next_qwen \
  --finetuning_type lora \
  --output_dir /mnt/nfs/wmd/model/MemRefine/MemRefine-IAD_LLaVA-OneVision-SI-7B_SFT_v1 \
  --overwrite_cache \
